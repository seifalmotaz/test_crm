...
export default function DealsTable({ deals }: { deals: Deal[] }) {
  ...
  const tabs = ['All', 'Closing Soon', 'At Risk']
  ...
  return (
    <div className="bg-card card-border rounded-2xl p-5 glow-blue">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <div className="flex gap-1">
            {tabs.map((f, i) => (
              <button key={f} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${i === 0 ? 'bg-blue-500 text-white' : 'text-slate-400 border border-white/10 hover:text-white hover:bg-white/5'}`}>
                {f}
              </button>
            ))}
          </div>
          <button onClick={() => navigate({ to: '/deals' })} className="flex items-center gap-1 text-blue-400 text-xs hover:text-[#E53935] transition-colors">
      ...
      <table className="w-full">
        ...
        <tbody>
          {deals.map((deal) => (
            <tr key={deal.id} onClick={() => navigate({ to: '/deals' })} className="border-b border-white/4 hover:bg-white/3 transition-colors group cursor-pointer">
